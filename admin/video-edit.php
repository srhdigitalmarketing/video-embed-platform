...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if(isset($_POST['stream_action'])){
   ...
   if($action==='add'){
     $label=trim($_POST['stream_label']??'Stream');$url=trim($_POST['stream_url']??'');
     if($url===''||!filter_var($url,FILTER_VALIDATE_URL)){$msg='A valid stream URL is required.';}else{
       $mx=db()->prepare('SELECT COALESCE(MAX(sort_order),-1)+1 FROM stream_links WHERE video_id=?');$mx->execute([$id]);$order=(int)$mx->fetchColumn();
       db()->prepare('INSERT INTO stream_links(video_id,label,url,status,sort_order) VALUES(?,?,?,?,?)')->execute([$id,$label?:'Stream',$url,1,$order]);log_action('stream.create','video_id='.$id);header('Location: video-edit.php?id='.$id.'#streams');exit;
     }
   } elseif($action==='update'){
     $label=trim($_POST['stream_label']??'Stream');$url=trim($_POST['stream_url']??'');
     if(!$sid||$url===''||!filter_var($url,FILTER_VALIDATE_URL)){$msg='A valid stream URL is required.';}else{
       db()->prepare('UPDATE stream_links SET label=?,url=?,status=? WHERE id=? AND video_id=?')->execute([$label?:'Stream',$url,isset($_POST['stream_status'])?1:0,$sid,$id]);log_action('stream.update','stream_id='.$sid);header('Location: video-edit.php?id='.$id.'#streams');exit;
     }
   ...
   if($title===''){$msg='Title is required.';}else{
     ...
     log_action('video.update','video_id='.$id);header('Location: video-edit.php?id='.$id);exit;
   }
 }
}
...
<div id="streams" class="panel stream-panel"><div class="panel-head"><div><h2>Stream Links</h2><span class="panel-subtitle">Manage fallback/alternate player sources.</span></div><button type="button" class="button" data-add-stream>+ Add Stream</button></div>
...
<div class="stream-grid"><input name="stream_label" value="<?=e($s['label'])?>" placeholder="Link label"><input name="stream_url" value="<?=e($s['url'])?>" placeholder="https://.../video.m3u8"><span class="status-pill <?=$s['status']?'on':'off'?>">Status : <?=$s['status']?'Active':'Inactive'?></span></div>
<div class="stream-actions"><label class="check"><input type="checkbox" name="stream_status" <?=$s['status']?'checked':''?>> Active</label><button class="button" type="submit">Save</button><button class="button danger" type="submit" name="stream_action" value="delete" onclick="return confirm('Delete stream link?')">Delete</button></div>
</form>
<?php endforeach;?>
<div id="stream-add" class="stream-add hidden"><form method="post"><?=csrf_field()?><input type="hidden" name="stream_action" value="add"><div class="stream-grid"><input name="stream_label" value="Link <?=count($streams)+1?>" placeholder="Link label"><input name="stream_url" placeholder="https://.../video.m3u8" required></div><div class="stream-actions"><button class="button" type="submit">Add Stream Link</button><button class="button secondary" type="button" data-cancel-stream>Cancel</button></div></form></div>
<?php if(!$streams):?><div class="empty">No stream links yet. Add an HLS (.m3u8) or MP4 stream URL.</div><?php endif;?>
</div></div>
