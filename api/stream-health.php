...
$type=(string)($data['event_type']??'timeout');
$allowed=['timeout','load_error','manual_switch','play_failed'];
if(!in_array($type,$allowed,true)||!$streamId||!$token){http_response_code(400);echo json_encode(['error'=>'invalid']);exit;}
$st=db()->prepare('SELECT t.id token_id,v.id video_id FROM embed_tokens t JOIN videos v ON v.id=t.video_id WHERE t.token=? AND t.status=1 LIMIT 1');$st->execute([$token]);$owner=$st->fetch();if(!$owner){http_response_code(403);echo json_encode(['error'=>'token']);exit;}
...
// Keep a recent error in admin logs so it appears in Recent Activity/notification counts.
db()->prepare("INSERT INTO system_logs(user_id,action,details,ip_hash) SELECT NULL,?,?,NULL")->execute(['stream.'.$type,'video_id='.$owner['video_id'].'; stream_id='.$streamId.'; host='.$stream['label']]);
